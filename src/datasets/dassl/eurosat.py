...
# Dictionary for new class names
NEW_CNAMES = {
...
class EuroSAT(Dataset):
    """
    A PyTorch Dataset for the EuroSAT dataset, which contains satellite images of various land cover types.
    This class handles loading the dataset, splitting it into train, validation, and test sets, and preprocessing the images.

    Attributes:
        dataset_dir (Path): The directory where the EuroSAT dataset is stored.
        ...
        classnames (List[str]): A list of class names for each land cover type.
    """

    dataset_dir = Path("eurosat")

    def __init__(self, dataroot: Path, split: str, preprocess: callable):
        """
        Initializes the EuroSAT dataset by loading and preprocessing the data for the specified split.

        Args:
            dataroot (Path): The root directory where the EuroSAT dataset is stored.
    ...
    def get_classnames(self, *args, **kwargs) -> List[str]:
        ...
        Returns:
            list: A list of class names for the land cover types.
        """
